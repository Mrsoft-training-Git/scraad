  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="grid lg:grid-cols-2 gap-4 md:gap-8">
        ...
        <Card className="border-none shadow-card">
          <CardHeader className="bg-foreground text-background p-4 md:p-6">
            <CardTitle className="text-base md:text-lg font-heading">Recent Payments</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[400px]">
                <thead className="bg-muted">
                  <tr>
                    <th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold">Payment</th>
                    ...
                    <th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold hidden lg:table-cell">Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {recentPayments.map((payment, index) => (
                    <tr key={index} className="hover:bg-muted/50 transition-colors">
                      <td className="px-3 md:px-6 py-3 md:py-4">
                        <div className="flex items-center gap-2 md:gap-3">
                          <div className="w-6 h-6 md:w-8 md:h-8 bg-accent rounded-full flex-shrink-0"></div>
                          <span className="font-medium text-sm md:text-base">{payment.user}</span>
                        </div>
                      </td>
                      <td className="px-3 md:px-6 py-3 md:py-4 text-xs md:text-sm font-semibold">{payment.amount}</td>
                      <td className="px-3 md:px-6 py-3 md:py-4 text-xs md:text-sm max-w-[150px] truncate hidden md:table-cell">{payment.course}</td>
                      <td className="px-3 md:px-6 py-3 md:py-4 text-xs md:text-sm text-muted-foreground hidden lg:table-cell">{payment.date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
